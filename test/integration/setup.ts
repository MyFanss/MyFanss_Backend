...
export async function createIntegrationApp(): Promise<IntegrationApp> {
  ...
  return {
    ...
    tokenRepo: module.get<Repository<RefreshToken>>(getRepositoryToken(RefreshToken)),
  };
}

export async function teardownIntegrationApp(ctx: IntegrationApp): Promise<void> {
  await ctx.app.close();
...
export async function clearAll(dataSource: DataSource): Promise<void> {
  await dataSource.query('TRUNCATE TABLE "refresh_tokens" RESTART IDENTITY CASCADE');
